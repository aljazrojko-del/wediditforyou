// POST /api/admin/leads/[slug]/call — place one personalized cold call via SignalWire.
...
export async function POST(
  ...
  const result = await placeCallToLead(data, client, supabase);
  if (!result.ok) {
    return NextResponse.json(
      { error: result.error ?? result.skipped ?? "call failed" },
      { status: 400 },
    );
  }
