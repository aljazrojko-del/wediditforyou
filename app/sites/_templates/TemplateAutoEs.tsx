...
// Spanish-localized clone of TemplateAuto (classic dark + orange). Same layout,
// translated static labels — so a Spanish-speaking lead gets a coherent page.
const inter = Inter_Tight({ subsets: ["latin"], weight: ["400", "500", "600", "700", "800", "900"] });

const DEFAULTS = {
  bg: "#0F1216", bgSurface: "#161A20", accent: "#FF6B00", accentHover: "#FFA250",
  accentContrast: "#0F1216", text: "#FFFFFF", textMuted: "#A0A6B0",
} as const;

type ThemeVars = {
  "--t-bg": string; "--t-bg-surface": string; "--t-accent": string; "--t-accent-hover": string;
  "--t-accent-contrast": string; "--t-text": string; "--t-text-muted": string;
};
...
export default function TemplateAutoEs({ data }: { data: SiteData }) {
  const tel = data.phone ? data.phone.replace(/[^0-9+]/g, "") : "";
  const phoneDisplay = data.phone ?? "Llámanos";
  ...
  return (
    <div style={themeStyle} className={`${inter.className} min-h-screen bg-[var(--t-bg)] text-[var(--t-text)]`}>
      <div className="bg-[var(--t-accent)] text-[var(--t-accent-contrast)]">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-2 text-sm font-bold uppercase tracking-wider">
          <span>⚙ Disponible ahora · llega en 30 min a {data.city}</span>
        ...
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-5">
          {data.logoUrl ? (
            <div className="flex items-center gap-3">
              <Image src={data.logoUrl} alt={`${data.businessName} logo`} width={56} height={56} className="h-14 w-14 rounded bg-white object-contain p-1" />
              <span className="hidden text-lg font-extrabold uppercase tracking-tight sm:inline">{data.businessName}</span>
            </div>
          ) : (
      ...
      <section className="mx-auto w-full max-w-6xl px-6 pt-16 pb-20 sm:pt-20">
        <div className="grid gap-12 lg:grid-cols-12 lg:gap-10">
          <div className="lg:col-span-7">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[var(--t-accent)]/40 bg-[var(--t-accent)]/10 px-3 py-1 text-xs font-bold uppercase tracking-widest text-[var(--t-accent)]">
              ...
              A domicilio · Precios claros
            </div>
            <h1 className="text-5xl font-extrabold uppercase leading-[0.95] tracking-tight sm:text-7xl">{data.headline}</h1>
            {data.subheadline && <p className="mt-8 max-w-xl text-lg text-[var(--t-text-muted)]">{data.subheadline}</p>}
          ...
          <div className="lg:col-span-5">
            <div className={`relative overflow-hidden rounded-2xl border-2 border-[var(--t-accent)]/30 bg-black/20 ${data.coverUrl ? "aspect-video" : "aspect-[4/5]"}`}>
              <Image src={data.heroImage} alt={data.businessName} fill priority sizes="(max-width: 1024px) 100vw, 40vw" style={data.coverUrl ? { objectFit: "contain" } : { objectFit: "cover" }} />
              {!data.coverUrl && (
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[var(--t-bg)] to-transparent p-5">
                  <div className="text-xs font-bold uppercase tracking-widest text-[var(--t-accent)]">En sitio en 30 min · {data.city}</div>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="mt-16 grid grid-cols-2 gap-6 border-t border-current/10 pt-8 sm:grid-cols-4">
          {[
            [data.rating ? `${data.rating.toFixed(1)}★` : "5.0★", "Reseñas Google"],
            [data.ratingCount ? `${data.ratingCount}+` : "100+", "Trabajos hechos"],
            ["30 min", "Respuesta prom."],
            [data.city.split(",")[0], "Zona"],
          ].map(([big, small]) => (
            <div key={small}>
      ...
      <section className="border-t border-current/10 bg-[var(--t-bg-surface)]">
        <div className="mx-auto w-full max-w-6xl px-6 py-20">
          <h2 className="mb-12 text-4xl font-extrabold uppercase tracking-tight sm:text-5xl">Lo que reparamos.</h2>
      ...
      <section className="border-t border-current/10 bg-[var(--t-bg-surface)]">
        <div className="mx-auto w-full max-w-6xl px-6 py-20">
          <div className="mb-10 flex items-end justify-between">
            <h2 className="text-4xl font-extrabold uppercase tracking-tight sm:text-5xl">Trabajos recientes.</h2>
            <span className="hidden text-xs font-bold uppercase tracking-widest text-[var(--t-accent)] sm:block">Esta semana · {data.city}</span>
          </div>
      ...
      <section className="border-t border-current/10">
        <div className="mx-auto w-full max-w-6xl px-6 py-20">
          <div className="mb-12 flex items-end justify-between gap-4">
            <h2 className="text-4xl font-extrabold uppercase tracking-tight sm:text-5xl">{data.city} confía en nosotros.</h2>
            <div className="hidden text-xs font-bold uppercase tracking-widest text-[var(--t-text-muted)] sm:block">{data.reviews.length} reseñas verificadas · desliza</div>
          </div>
          <div className="flex snap-x snap-mandatory gap-5 overflow-x-auto pb-5" style={{ scrollbarWidth: "thin", WebkitOverflowScrolling: "touch" }}>
            {data.reviews.map((r) => (
              <figure key={r.name} className="flex min-h-[240px] w-[85%] shrink-0 snap-start flex-col border border-current/10 bg-[var(--t-bg-surface)] p-7 sm:w-[420px]">
                <div className="mb-3 text-[var(--t-accent)]">★★★★★</div>
                <blockquote className="flex-1 text-[var(--t-text)]">&ldquo;{r.quote}&rdquo;</blockquote>
                <figcaption className="mt-5 text-xs uppercase tracking-widest text-[var(--t-text-muted)]">{r.name} · Reseña de Google</figcaption>
              </figure>
            ))}
          </div>
        </div>
      ...
      <section id="book" className="border-t border-current/10 bg-[var(--t-accent)] text-[var(--t-accent-contrast)]">
        <div className="mx-auto flex w-full max-w-6xl flex-col items-start justify-between gap-6 px-6 py-16 sm:flex-row sm:items-center">
          <div>
            <h2 className="text-4xl font-extrabold uppercase leading-tight tracking-tight sm:text-5xl">¿Te quedaste varado?</h2>
            <p className="mt-3 text-lg font-medium">Dinos dónde estás. Llegamos en 30 minutos.</p>
          </div>
          {tel && (
      ...
      <footer className="bg-[var(--t-bg)]">
        <div className="mx-auto grid w-full max-w-6xl gap-8 px-6 py-12 sm:grid-cols-3">
          <div>
            ...
            <p className="text-sm text-[var(--t-text-muted)]">A domicilio. Precios claros, siempre.</p>
          </div>
          {data.address && (
            <div>
              <div className="mb-2 text-xs font-bold uppercase tracking-widest text-[var(--t-text-muted)]">Zona de servicio</div>
              <p className="text-sm text-[var(--t-text)]">{data.city}</p>
            </div>
          )}
          {tel && (
            <div>
              <div className="mb-2 text-xs font-bold uppercase tracking-widest text-[var(--t-text-muted)]">Llamar</div>
              <a href={`tel:${tel}`} className="text-sm font-bold text-[var(--t-accent)]">{phoneDisplay}</a>
            </div>
          )}
        </div>
        <div className="border-t border-current/10">
          <div className="mx-auto w-full max-w-6xl px-6 py-5 text-xs text-[var(--t-text-muted)]">© {new Date().getFullYear()} {data.businessName}</div>
        </div>
      </footer>
    </div>
  );
