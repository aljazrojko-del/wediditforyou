# Sales Call Playbook — wediditforyou

**Audience:** US local service businesses with strong Google reviews and no website.
**Locked niches:** mobile mechanics, mobile dog groomers, tutors.
**Offer:** $297 one-time. Site already built. They pay only if they keep it.
**Persona:** Alex.

This is the live script. Update it after every 25 calls based on what actually converts.

---

## 1. Pre-call prep — 60 seconds before dialing

Open the prospect's row in your CSV. Read out loud:

- [ ] First name of owner (use it in the first 10 seconds)
- [ ] Business name (don't mangle it)
- [ ] Star rating + number of reviews ("4.8 with 67 reviews" — these become social proof)
- [ ] What they do specifically (mobile mechanic for diesel trucks? mobile groomer for cats too? tutor for SAT or middle school math?)
- [ ] City
- [ ] Best detail from a recent 5-star review (one phrase you can quote in the demo book)

If you can't find at least 4 of these in 60 seconds, skip that prospect. Move to next. Bad prep = burnt prospect.

---

## 2. Opener — pick the script that matches the niche

### Mobile mechanic

> "Hey [first name], this is Alex. Quick question — I was trying to find your prices online before I called and I couldn't see a website anywhere. Is that on purpose?
>
> [pause for response]
>
> No sales pitch coming. Reason I'm calling: I saw your reviews on Google — [4.8 with 67 reviews / specific stat] — and figured if you've got that kind of reputation and no website to capture the people Googling you, that's leaving money on the table. So I went ahead and built you one. Free to look at. If you like it, $297. If you don't, you keep the draft anyway. Worth 10 minutes tonight?"

### Mobile dog groomer

> "Hey [first name], this is Alex. Quick question — I tried to book you online for my dog and there's no website. Is that on purpose?
>
> [pause]
>
> No pitch. I called because your Google reviews are 5-star across the board and you don't have a site for new clients to actually find you and book. So I built you one already. Free to look at. $297 if you like it, nothing if you don't. Can you do 10 minutes tonight to see it?"

### Tutor

> "Hey [first name], this is Alex. I was looking for a [SAT / math / reading] tutor in [city] for [my niece / my kid / a friend] and saw your reviews — parents are raving — but there's no website. Did that surprise me. So I went ahead and built you one already. Free to look at. $297 if you keep it, nothing if you don't. Got 10 minutes tonight?"

**Tone rules:**
- Stay calm. Don't oversell. Belfort straight-line tonality — confident, not loud.
- Match their register. If they say "yeah man" — you say "yeah man". If they're formal — you go formal.
- After the opener, **shut up**. Let them respond. Don't fill silence.

---

## 3. Branch paths

### Path A — They bite ("Sure, send me the link" / "What is it?")

> "Yeah I'll send you a Google Meet link in 60 seconds, easier to show you than explain. Are you near a phone screen right now or do you want me to call in an hour?"

Book the slot. Hang up. Send the link via SMS within 60 seconds. **If you delay, they bail.**

### Path B — "I get these calls all the time, not interested"

> "Yeah, I'd hang up on me too. Reason this is different — I'm not asking you to buy anything. The site is already built. It costs you zero to look. If you hate it, hang up on me and I'll never call again. If you like it, $297. The risk on your side is 10 minutes of your time. Worth that?"

If still no → "All good. Last thing — can I text you the link? Look at it whenever. If you want to chat, text me back." (gets you the SMS opt-in)

### Path C — "Too expensive"

You haven't quoted yet — they're objecting to an imagined number. Anchor first:

> "Fair — what number were you imagining? Most local agencies charge $1,500 to $5,000 for a site like this. I do it for $297 because I'm not running an agency, I'm running software. You're paying me to press a button. The site exists already. If $297 is still too expensive, no problem — but I bet you've spent more than that on a single tow truck repair, right?"

Wait for the laugh. Move to demo book.

### Path D — "I don't have time"

> "That's literally the point of having a website. Right now every call you miss is a job you lost. The site books appointments while you're under a hood. Look, 10 minutes tonight. I'll show you, you decide. Or text me a time that works."

### Path E — "What's the catch?"

> "There isn't one. The catch on my side is I bet you'll like the site. If 1 in 10 people I call keeps the site, I make money. I'm not asking for a deposit, contract, anything. You see it, you decide."

### Path F — "I already have someone working on it / had a bad experience"

> "Got it — what happened?"

LISTEN. This is golden. Whatever they say (took too long, never delivered, bait-and-switch on price), echo it back:

> "So [agency] took six months and never went live. That's exactly why I built this. The site is already done. You see it tonight, it's live tomorrow if you want it. Zero risk on your side. Worth 10 minutes?"

### Path G — "Send me an email"

This is "no" in a polite sweater. Try once:

> "Yeah I can — but heads up, the email is just a Google Meet link to look at the site I built. Two minutes faster to do it now. Open to a quick look?"

If they push back → take the email, send the nurture sequence (§7). Don't fight it.

---

## 4. Discovery questions (use on the demo book call, not the opener)

Voss-style calibrated questions. Get them talking about pain in their own words. **You shut up and write down their phrasing — use it back later.**

- "How are you currently getting new clients?"
- "What does it cost you when you miss a call during a job?"
- "How many bookings a week feels healthy for you?"
- "What's the part of the booking process you'd most want to disappear?"
- "Walk me through the last time a customer tried to book and couldn't — what happened?"

The last one is the closer. If they have a story, you've got pain. If they don't, the offer is harder.

---

## 5. Demo book close — the 10-minute call

Show the actual site you built for them. Tell them three things in this order:

1. **What the site does that they can't do today.** ("This page lets a customer pick a time on your calendar and pay a deposit before you ever pick up the phone.")
2. **One specific decision they get to make.** ("I made the call-to-action say 'Book a service call' — you want that, or want it to say 'Get a quote'?") — gives them ownership.
3. **The close itself.** "If everything looks good and we change the button text, are you keeping it?"

Wait for the answer. **Don't keep selling.**

If yes → "Cool. I'll send a Stripe link right now for $297, and the site goes live on your domain tonight. Do you have a domain or do you want me to register one?"

If maybe → "What would make this a clear yes?" Then handle that one thing. Then re-ask the close.

If no → "Got it. What didn't land?" Listen. Don't rebut. Thank them. Add to nurture.

---

## 6. Voicemail script (for the 60% who don't pick up)

Keep it under 25 seconds. Two parts: pattern interrupt + reason to call back.

> "Hey [first name], this is Alex. I built you a website. Free to look at — paid 297 if you keep it, nothing if you don't. No catch, no contract. Texting you the link right now. Call me back when you've seen it. Number's 210-555-XXXX."

Hang up. Send the SMS in §7 immediately.

---

## 7. SMS follow-up sequence

Send this from the OpenPhone number with the matching area code. Each message stands alone — don't reference earlier ones.

### T+0 (right after the voicemail or after Path G email)

> Hey [first name] — Alex. Just left you a voicemail. Built you a site, free to look at. Link: [demo URL]. $297 if you keep it. Text back anytime.

### T+24h (if no reply)

> [First name] — peek at it? Took me 4 minutes to build, you'll spend 2 minutes looking. [demo URL]

### T+72h

> [First name] — sounds like the timing's off. No worries. Quick question: if you DID get a website, what's the one thing you'd want it to do? Curious.

(This is a re-engagement bait — asks for opinion, not money. Most who reply convert later.)

### T+7d

> [First name] — last one from me. Site I built is here if you want it: [demo URL]. After today I take it offline. Best of luck either way. — Alex

After T+7d: stop. Don't be the person who texts 12 times. Move on.

---

## 8. Numbers to track per week

Write these in a sheet. If you don't track them you can't fix them.

| Metric | Target (first month) |
|---|---|
| Dials per day | 30 |
| Connect rate (live human picks up) | ≥ 30% |
| Demo books per week | ≥ 5 |
| Demos → close | ≥ 40% |
| Closes per week | ≥ 2 |

If connect rate is below 30%: you're calling at the wrong time. Try 4–6 PM local for service businesses.
If demo-book rate is below 25% of connects: opener is broken. Re-record and re-test.
If demo → close is below 30%: site quality issue or pricing isn't landing. Look at the demo replay.

---

## 9. Banned phrases and behaviors

Things that work for Nick (the transcript I studied) but will burn YOUR niches:

- **Don't be confrontational.** No "you feeling okay?" jabs. Mobile dog groomers and tutors will hang up. Mobile mechanics will tolerate it but you don't need it.
- **Don't bait-and-switch the opener.** No "I was hunting down a contract" / pretending to be a customer. Open honest. They respect the honesty.
- **Don't quote different prices to different prospects.** Always $297. Quoting $500 once and $297 next looks made-up.
- **Don't oversell.** Once they say yes, shut up and take the money. Talking past the close is the #1 way deals die.
- **Don't use** these words: unleash, leverage, revolutionize, world-class, premium, cutting-edge, synergy, "Welcome to". They're tells that you're scripted.

---

## 10. Quick reference card (cut this out, tape it to your monitor)

```
OPENER:  Honest. Compliment their reviews. State the offer in one breath.
PRICE:   $297. Anchor $1.5k–$5k. Never quote anything else.
RISK:    Site built. Pay only if you keep it.
CLOSE:   "If everything looks good, are you keeping it?"
LIMIT:   5 sites a week. 2 slots left this week.  (REAL — don't lie)
NEXT:    Stripe link → site live tonight on their domain.
SILENCE: After the close question, count to 7 in your head. SHUT UP.
```

— Alex
